...
const ManagerProfile = () => {
  ...
  const handleProfileSave = (e) => {
    ...
    setTimeout(() => {
      ...
      // Force custom event update to header/sidebar
  ...
  return (
    <div className="d-flex flex-column gap-lg" style={{ maxWidth: '800px', margin: '0 auto' }}>
      <div>
        <h2 className="text-bold" style={{ fontSize: '22px' }}>My Profile Details</h2>
        <p className="text-secondary-color" style={{ fontSize: '14px' }}>
          Inspect credentials and edit contact points linked to AgriFreeze supervisor rosters
        </p>
      </div>

      {success && (
        <div className="card-base bg-success-light p-md animate-fade-in" style={{ border: '1px solid rgba(34, 197, 94, 0.2)' }}>
          <span className="text-semibold" style={{ color: 'var(--dark-green)', fontSize: '14px' }}>Profile updated successfully.</span>
        </div>
      )}

      <div className="d-grid grid-cols-3 gap-md">
        {/* Left Card: supervisor Info Card */}
          ...
          <div className="card-base text-center p-lg d-flex flex-column align-center gap-md">
            ...
            <div className="d-flex flex-column gap-xs text-left" style={{ width: '100%', borderTop: '1px solid var(--border)', paddingTop: 'var(--spacing-md)' }}>
              <div className="d-flex align-center gap-sm text-secondary-color" style={{ fontSize: '12px' }}>
                <ShieldCheck size={14} />
                <span>Duty Facility: <span className="text-semibold" style={{ color: 'var(--text-primary)' }}>{user.facility || 'Alpha & Gamma'}</span></span>
              </div>
              <div className="d-flex align-center gap-sm text-secondary-color" style={{ fontSize: '12px', marginTop: '4px' }}>
                <Calendar size={14} />
                <span>Joined On: <span className="text-semibold" style={{ color: 'var(--text-primary)' }}>{user.joinDate}</span></span>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="card-base p-lg">
            ...
            <form onSubmit={handleProfileSave} className="d-flex flex-column gap-md">
              ...
              <div className="d-grid grid-cols-2 gap-md">
                <div className="form-group">
                  <label htmlFor="prof-username">Sign In Username</label>
                  <input 
                    id="prof-username"
                    type="text" 
                    value={user.username} 
                    disabled
                    className="login-input"
                    style={{ paddingLeft: '14px', height: '42px', backgroundColor: 'var(--background)', cursor: 'not-allowed' }}
                  />
                </div>

                <div className="form-group">
                  <label htmlFor="prof-role">Assigned System Role</label>
                  <input 
                    id="prof-role"
                    type="text" 
                    value={user.role} 
